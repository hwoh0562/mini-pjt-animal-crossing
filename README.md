...
#### 측정의 한계 — 점수를 그대로 믿으면 안 되는 지점

- **`faithfulness` 0.882 는 10건이 아니라 9건 평균입니다.** R6 에서 판정자가
  `OutputParserException` 을 내 그 샘플이 집계에서 빠졌습니다. 재현되는 실패라 원인을
  좁히지 못했고, 표본이 하나 적은 상태의 값입니다.
- **`context_precision` 은 집계형 질의를 과소평가합니다.** 이 지표는 검색된 문서를 하나씩
  ...
  지표가 다건 집계를 다루는 방식의 한계입니다. 정답 문구를 길게 고치면 점수는 오르지만
  그건 측정 대상이 아니라 자를 바꾸는 것이라 두었습니다.
- **`answer_relevancy` 는 언어에 의존합니다.** 답변에서 질문을 역생성해 원 질문과 코사인
  유사도를 재는데, RAGAS 기본 프롬프트가 영어라 역생성 질문이 영어로 나왔습니다. Titan
  임베딩에서 영↔한 유사도는 0.278(한↔한 패러프레이즈는 0.607~0.764)이라 완벽한 답변도
  0.25 근처가 나왔습니다. 한국어로 뽑도록 지시해 그 실험에서 0.419 → 0.922 로 정상화했고
  현재 값은 0.914 입니다. 다국어 환경에서는 같은 함정이 반복될 수 있습니다.
- **LLM-as-Judge 판정이 실행마다 흔들립니다.** 같은 답변에 대해 id 4(시간대 위반 종 혼입)와
  id 12(대안 제시)의 판정이 회차마다 뒤집힌 적이 있습니다. 사실 관계는 코드로 옮겨
  (`expected_tools` 대조, id 13 의 Store 확인, id 4 의 시간대 검증) 흔들림을 줄였지만,
  `expected_traits`·`forbidden` 은 여전히 LLM 판정이라 **20/20 이 매 실행 보장되지는
  않습니다.**

#### 운영·인프라

- **Bedrock 일일 토큰 한도** — 평가 한 바퀴가 LLM 호출 90~180회라, 하루에 RAGAS·인-아웃
  세트·시연을 모두 돌리면 `ThrottlingException` 이 납니다. 적응형 재시도
  (`max_attempts=10, mode=adaptive`)와 `SQLiteCache` 로 완화했지만 반복 실행은 여전히
  쿼터에 묶입니다.
- **Docker 이미지의 `/query` 검증 미완** — 빌드·기동·색인 128건·`/health` 까지 확인했고
  컨테이너 안에서 검색도 정상 동작합니다. 다만 LLM 을 타는 `/query` 는 두 차례 시도 모두
  일일 토큰 한도에 걸려 응답을 받지 못했습니다. Docker 문제가 아니라 쿼터 문제이며, 같은
  코드가 로컬에서 20/20 으로 통과합니다. 컨테이너는 `llm_cache.sqlite` 를 이미지에서
  제외하므로 로컬보다 호출이 많아 더 불리합니다.
- **Python 3.14 에서 RAGAS 상위 API 를 못 씁니다.** `evaluate()` 와 `single_turn_ascore()`
  가 내부에서 `asyncio.wait_for` 를 써 `Timeout should be used inside a task` 로 전부
  실패합니다. 타임아웃을 두르지 않는 `_single_turn_ascore` 를 직접 호출해 우회했고 지표
  계산 로직은 건드리지 않았지만, 라이브러리가 의도한 진입점을 쓰지 못하는 상태입니다.

#### 서비스 범위

- **보유하지 않은 데이터** — 해산물, 생물의 출현 월·계절·반구·날씨. 질의가 오면 추측하지
  않고 "확인되지 않는다"로 안내합니다. 생물의 장소·시간대·판매가도 이름만 실제 명칭이고
  값은 시드 고정으로 생성한 더미입니다.
- **단일 사용자 전제** — `POST /query` 요청 본문에 `thread_id` 가 없어 스레드를 하나로
  고정했습니다. 다중 사용자로 확장하려면 인증 주체별로 스레드를 나눠야 합니다.
- **Chroma HNSW 는 근사 탐색입니다.** 색인을 새로 만들면 순위가 미세하게 달라집니다.
  Docker 이미지에서 실제로 `1호` 질의의 하이브리드 1위가 `V-16` → `P-06` 으로 바뀌었습니다
  (RRF 0.0328 vs 0.0321). 최종 순서는 리랭킹이 정하므로 서비스 동작에는 영향이 없지만,
  색인 직후 스모크 테스트가 1위를 단정하던 것은 과한 가정이라 상위 3건 포함으로 낮췄습니다.

### 향후 개선 방향

- 생물 데이터에 출현 월·반구를 추가해 계절 질의를 지원
- R6 의 `faithfulness` 판정이 재현적으로 실패하는 원인 규명 — 판정 프롬프트가 다건 집계
  답변을 파싱하지 못하는 것으로 보이나 확인 필요
- 평가를 여러 번 돌려 통과율의 분산을 재고, 흔들리는 케이스의 기대값을 코드 검증으로 이관
- 컨테이너에도 LLM 캐시 볼륨을 붙여 Docker 환경의 반복 검증 비용을 낮추기
